...
보통 먼저 xschem상에서 회로설계를 어느정도 진행하고 그 후 파이썬 프로그래밍을 통해 최적의 파라미터(R,tr_width,tr_length,...)를 결정한다고 생각하면, **회로설계에서 파이썬 으로 전환하는 시점**에서는 **2번에서 3번으로 netlist 파일형식을 변화**시켜주는 작업이 필요합니다. 잊지말고 **`batch mode`에서 허용되는 command를 참고하여 형식을 변화**를 해주시기 바랍니다.(`batch mode`커맨드는 ngspice_manual 참고)